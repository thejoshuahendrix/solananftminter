...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getMintedNFTs = useCallback(async () => {
    ...
    try {
      const nfts = [];
      let first = await fetch("https://arweave.net/gkqQYMH9LyZH9jqVsYCrrm-JUohSmVYWWSmBZJZZd4I");
      nfts.push(await first.json());
      let second = await fetch("https://arweave.net/y_DpK6pSzH_tQe_EtFEdZN_yORj4yD0v_2gsPeyqUW0");
      nfts.push(await second.json());
      let third = await fetch("https://arweave.net/DsMcMXOS4f3Qph-RoUKERRYMBZgemsx66RGwTs0XaEc");
      nfts.push(await third.json());
      setMints(nfts);
    } catch (e) {
  ...
  const renderMintedItems = () => (
    <div className="gif-container">
      <p className="sub-text">Minted Generations ✨</p>
      <div className="gif-grid">
        {mints.map((mint, i) => (
          <div className="gif-item" key={i}>
            <img src={mint.image!= "1.png"?mint.image:mint.properties.files[0]['uri']} alt={`Minted NFT ${i}`} />
          </div>
        ))}
      </div>
    </div>
